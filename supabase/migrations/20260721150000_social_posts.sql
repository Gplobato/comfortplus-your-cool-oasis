-- Social content planner: schedule organic posts for Instagram / Facebook.
-- Reuses the creatives library as the media source and stays scoped per organization.
CREATE TABLE public.social_posts (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  organization_id uuid NOT NULL REFERENCES public.organizations(id) ON DELETE CASCADE,
  creative_id uuid REFERENCES public.creatives(id) ON DELETE SET NULL,
  -- Where the post goes: instagram_feed | instagram_story | facebook_feed
  ...
  -- Snapshot of the media at scheduling time (creatives may change/be archived)
  media_url text,
  storage_path text,
  media_type text,
  ...
  -- Reserved for phase 2 auto-publishing via Meta Content Publishing API
...
CREATE INDEX social_posts_org_schedule_idx
  ON public.social_posts (organization_id, scheduled_for);
CREATE INDEX social_posts_org_status_idx
  ON public.social_posts (organization_id, status);
CREATE INDEX social_posts_creative_idx
  ON public.social_posts (creative_id);
